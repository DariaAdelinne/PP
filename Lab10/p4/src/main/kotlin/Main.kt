...
fun main(args: Array<String>) {
    //preluam calea directorului din args sau folosim implicit "Fisiere"
    ...
    //listam toate fisierele din director
    ...
    //cream un thread-pool fix cu 4 thread-uri
    val poolSize = 4
    val pool = Executors.newFixedThreadPool(poolSize)

    println("Incep procesarea a ${files.size} fisiere pe $poolSize thread-uri...\n")

    //pentru fiecare fisier, submit-iam un Runnable care il proceseaza
    files.forEach { file ->
        pool.submit {
    ...
    //nu mai acceptam sarcini noi si asteptam finalizarea
    pool.shutdown()
